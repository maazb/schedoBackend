...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(email=user.email,password = passHash,contact = user.contact, name = user.name, image = user.image, fcmId = user.fcmId, meetingCal = user.meetingCal, eventCal = user.eventCal, newMeetingsOnHome = user.newMeetingsOnHome, newMessagesOnHome = user.newMessagesOnHome, newMessageNotifications = user.newMessageNotifications, newMeetingNotifications = user.newMeetingNotifications , requested = user.requested, added = user.added)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
